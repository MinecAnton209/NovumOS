...
### Quantum RNG (Module `quantum`)
*Requires `import "quantum"`*

| Function | Description | Example |
|----------|-------------|---------|
| `quantum.rand()` | Returns a random byte (0-255) from qubit measurement | `set r = quantum.rand();` |
| `quantum.entangle()` | Returns an entangled qubit pair packed into one int (low byte = first measurement, high byte = entangled partner) | `set p = quantum.entangle(); set a = p & 0xFF; set b = (p >> 8) & 0xFF;` |
| `quantum.info()` | Returns a status string showing RDRAND availability | `print(quantum.info());` |

Quantum random numbers are generated by simulating qubit superposition and measurement. When RDRAND is available (modern x86 CPUs), it derives entropy from thermal noise in silicon — quantum in nature. Without RDRAND, the system falls back to RDTSC jitter mixing.
...
| `math.round(n)` | Round to nearest integer | `math.round(3.5); // 4` |
...
### Example: Quantum Randomness
```nova
import "quantum";

// Random byte from qubit measurement
set r = quantum.rand();
print("Random byte: " + int(r));

// Entangled qubit pair (Bell state |00> + |11>)
set p = quantum.entangle();
set a = p & 0xFF;
set b = (p >> 8) & 0xFF;
print("Qubit A: " + int(a) + ", Qubit B: " + int(b));
if a == b {
    print("[CORRELATED]");
} else {
    print("[decoherence]");
